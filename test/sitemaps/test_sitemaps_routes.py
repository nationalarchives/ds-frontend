...
class SitemapsBlueprintTestCase(unittest.TestCase):
    ...
    def test_sitemap_index(self):
        with aioresponses() as m:
            ...
            mock_endpoint = (
                f"{self.mock_api_url}/pages/" "?format=json&offset=0&limit=1"
            )
            mock_respsone = {
    ...
    def test_sitemap_page_1_xml(self):
        with aioresponses() as m:
            domain = self.domain.replace("http://", "https://")
            mock_endpoint = (
                f"{self.mock_api_url}/pages/" "?format=json&order=id&offset=0&limit=500"
            )
            mock_respsone = {
                "meta": {"total_count": 3},
                "items": [
